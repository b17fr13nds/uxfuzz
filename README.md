...
you're completely free in choosing how the kernel should be running. however make sure to have qemu exit on a kernel panic or similar. the `fuzzer` binary should be placed in the directory you're in at OS startup. be careful to use the modified qemu emulator. (located in `./tools/qemu-7.1.0/build/`)
